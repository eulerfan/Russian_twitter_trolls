...
testSparse$r_nr<-factor(testSparse$r_nr,levels =c(0,1))
troc2<-prediction(tweet.logit.test,testSparse$r_nr)

#calculate the area under the ROC
auc2<-performance(troc2,measure = "auc",x.measure = "cutoff")


perf_r2<- performance(troc2, measure ='tpr',x.measure='fpr')
plot(perf_r2,colorize=T, main="Logistic Regression")
...
predtcart<-predict(tweetcart,newdata=testSparse,type = "class")
cartable<-table(testSparse$r_nr,predtcart)

print(cartable)
#Random Forest
library(randomForest)
russianForest<- randomForest(r_nr~.,data=trainSparse,nodesize=25,ntrees=200,na.action=na.roughfix)
predictForest<-predict(russianForest, newdata=testSparse)

table(testSparse$r_nr,predictForest)
